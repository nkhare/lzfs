...
//	root_inode = &root_vnode->v_inode; 
	...
	printk("inode is %lu root_vnode %p  root_inode %p", root_inode->i_ino,root_vnode, root_inode);
/*
	printk("root_vnode %p v_file %p function %s \n", root_vnode,&root_vnode->v_file,__FUNCTION__);
        printk("root_vnode %p v_lock %p function %s \n", root_vnode,&root_vnode->v_lock,__FUNCTION__);
        printk("root_vnode %p  v_flag %p function %s \n", root_vnode,&root_vnode-> v_flag,__FUNCTION__);
        printk("root_vnode %p v_data %p function %s \n", root_vnode,&root_vnode->v_data,__FUNCTION__);
        printk("root_vnode %p v_vfsp %p function %s \n", root_vnode,&root_vnode->v_vfsp,__FUNCTION__);
        printk("root_vnode %p v_stream %p function %s \n", root_vnode,&root_vnode->v_stream,__FUNCTION__);
        printk("root_vnode %p v_type %p function %s \n", root_vnode,&root_vnode->v_type,__FUNCTION__);
        printk("root_vnode %p v_rdev %p function %s \n", root_vnode,&root_vnode->v_rdev,__FUNCTION__);
        printk("root_vnode %p  v_gfp_mask %p function %s \n", root_vnode,&root_vnode-> v_gfp_mask,__FUNCTION__);
        printk("root_vnode %p v_inode %p function %s \n", root_vnode,&root_vnode->v_inode,__FUNCTION__);

	printk("mutext size %lu  function %s \n", sizeof(kmutex_t), __FUNCTION__ );
*/
	root_dentry = d_alloc_root(root_inode);
	printk("root_dentry address %p in %s \n", root_dentry, __FUNCTION__);
	if (!root_dentry) {
	    ...
	    void *data, struct vfsmount *mnt)
{
	...
	zfs_register_callbacks(vfsp); /* While merging Neep*/
/*
